...
#if STARTUP_CONF_VERBOSE
    putstring("configured with address ");
...
main(void)
{
    ...
    //netstack_init();                       //edited by dongbo huang

    NETSTACK_RADIO.init();       //added by dongbo huang
    set_rime_addr();  //edited by dongbo huang
    ...
    //----------for list initializing------
    list_init(packetbuf_list);
    //

    autostart_start(autostart_processes);  //here just for text, it should not be commented! edited by dongbo huang
    ...
    while(1) 
    {
        ...
        if(process_getSleepCMD()==1)
        {
            ...
            clock_delay_usec(65);
            cc2530_rf_driver.on();
            process_setSleepCMD(0);
            /*fade(LEDS_RED);*/
            fade(LEDS_GREEN);
            putstring("woke up\r\n");
            printf("current in sleep mode is %d\r\n",irq_energest);
        }
        ...
        /*while(!(SLEEPCMD & SLEEP_HFRC_STB));  [> Wait for RCOSC to be stable <]*/
        /*CLKCON |= CLKCONCMD_OSC;               [> Switch to the RCOSC <]*/
